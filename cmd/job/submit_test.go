...
func TestSubmitCmd_PathwaysMTCFlags(t *testing.T) {
	...
	_, err := executeCommand(JobCmd,
		...
		"--compute-type", "n2-standard-4",
		"--gke-mtc-enabled",
		"--gke-ramdisk-dir", "/tmp/custom_mtc_dir",
	)
	...
	if gkeRamdiskDirectory != "/tmp/custom_mtc_dir" {
		t.Errorf("expected gkeRamdiskDirectory to be /tmp/custom_mtc_dir, got %s", gkeRamdiskDirectory)
	}
