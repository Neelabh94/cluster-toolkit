...
*   **Contents:**
    ...
    *   `runRunCmd(cmd *cobra.Command, args []string)`: The core execution function for the `run` command. It collects flag values into a `run.RunOptions` struct and then delegates the entire workflow orchestration to `run.ExecuteRun()`.
*   **Relationship:** Imports `hpc-toolkit/pkg/run` (the orchestration package) and `github.com/spf13/cobra`. It's the CLI interface to the `gcluster run` logic.
...
*   **Contents:**
    *   `RunOptions` struct: Holds all parameters gathered from the CLI for the `run` command.
    *   `ExecuteRun(opts RunOptions) error`: The main orchestration function.
        *   Retrieves the GCP `ProjectID` (either from `opts` or by calling `gcloud config get-value project`).
        *   Constructs `cloudbuild.BuildOptions` and calls `cloudbuild.GenerateCloudBuildYaml` and `cloudbuild.SubmitCloudBuild` to initiate the image build.
        *   Constructs the full image name, assuming a successful build.
        ...
        *   Either saves the manifest to a file (if `--output-manifest` is provided) or calls `gkemanifest.ApplyGKEManifest` to deploy the job to GKE.
*   **Relationship:** Imports and coordinates `hpc-toolkit/pkg/run/cloudbuild` and `hpc-toolkit/pkg/run/gkemanifest`. Also uses `hpc-toolkit/pkg/shell` for executing `gcloud` commands.
...
*   **Contents:**
    ...
    *   `GetFullImageName(imageName, projectID, region string) (string, error)`: A utility function to construct the fully qualified Docker image name (e.g., `gcr.io/<PROJECT_ID>/<IMAGE_NAME>:<TAG>`), handling cases where the input `imageName` may or may not include the project ID.
*   **Relationship:** Used by `hpc-toolkit/pkg/run/run.go` to handle image building. Relies on `hpc-toolkit/pkg/shell` for command execution.
...
*   **Contents (Additions):**
    ...
    *   `ExecuteCommand(command string) Result`: Executes a given shell command using `bash -c` and captures its output and exit code into a `Result` struct.
    *   `RandomString(length int) string`: Generates a random string of specified length, used for unique job names.
*   **Relationship:** These additions serve as a foundational utility layer used by `hpc-toolkit/pkg/run/cloudbuild`, `hpc-toolkit/pkg/run/gkemanifest`, and `hpc-toolkit/pkg/run/run.go` for all shell interactions.
...
2.  **GCP Project ID Resolution:** If not explicitly provided, the GCP Project ID is determined from the `gcloud` configuration.
3.  **Cloud Build Initiation:**
    *   A `cloudbuild.yaml` file is dynamically generated based on `--dockerfile`, `--build-context`, and the determined image name.
    *   This `cloudbuild.yaml` and the build context are submitted to Google Cloud Build using `gcloud builds submit`. This initiates the Docker image build and push to a container registry (e.g., GCR).
    *   *(Note: For this POC, `gcluster run` does not wait for the Cloud Build to complete; it proceeds assuming the build will succeed.)*
...
| **Image Building**           | Cloud Build (via `gcloud builds submit`)             | Local Docker build (`docker buildx build`) + `docker push` |
